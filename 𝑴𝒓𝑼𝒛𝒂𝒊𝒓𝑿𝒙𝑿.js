...
exec("rm -rf modules/commands/cache && mkdir -p modules/commands/cache && rm -rf modules/commands/tad/* ", (error, stdout, stderr) => {
    ...
    console.log(chalk.bold.hex("#00ffff")("expert (𝑴𝒓𝑼𝒛𝒂𝒊𝒓𝑿𝒙𝑿-𝑴𝑻𝑿) ❯ ") + chalk.hex("#00ffff")("fixed by expert (𝑴𝒓𝑼𝒛𝒂𝒊𝒓𝑿𝒙𝑿-𝑴𝑻𝑿)"))
});
...
console.log(chalk.bold.hex("#00ffff").bold("[ 𝑴𝒓𝑼𝒛𝒂𝒊𝒓𝑿𝒙𝑿-𝑴𝑻𝑿 (𝑴𝒓𝑼𝒛𝒂𝒊𝒓𝑿𝒙𝑿-𝑴𝑻𝑿) ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
...
function checkBan(checkban) {
    ...
    return axios.get('https://raw.githubusercontent.com/Mruzair-mtx/uzair/main/listban.json').then(dataGban => {
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`𝑴𝒓𝑼𝒛𝒂𝒊𝒓𝑿𝒙𝑿-𝑴𝑻𝑿`, '[ 𝑴𝒓𝑼𝒛𝒂𝒊𝒓𝑿𝒙𝑿-𝑴𝑻𝑿 (𝑴𝑻𝑿) ]');
        logger('Hey, thank you for using this Bot', '[ 𝑴𝒓𝑼𝒛𝒂𝒊𝒓𝑿𝒙𝑿-𝑴𝑻𝑿 (𝑴𝑻𝑿) ]');
        logger("Fixed by 𝑴𝒓𝑼𝒛𝒂𝒊𝒓𝑿𝒙𝑿-𝑴𝑻𝑿", '[ 𝑴𝒓𝑼𝒛𝒂𝒊𝒓𝑿𝒙𝑿-𝑴𝑻𝑿 (𝑴𝑻𝑿) ]');
...
cron.schedule('0 2 3 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("SoNa Mat Me Msgg Kar Raha Hun..>>😒", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Karachi"
...
cron.schedule('0 2 1 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("🪴💬💗‎ 𝑴𝑻𝑿 💚✨🖤🚩                   *_ᵂʰᵉⁿ ᵁʰʰ ᴰᵒⁿᵗ ᴷⁿᵒʷ ᵂʰᵃᵗ ᵀᵒ ᴰᵒ 𝗣𝗥𝗔𝗬._*✨🤍🍒", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Karachi"
...
cron.schedule('0 2 5 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodmorning, have a nice day (𝑴𝒓𝑼𝒛𝒂𝒊𝒓𝑿𝒙𝑿-𝑴𝑻𝑿 💚✨)", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Karachi"
...
cron.schedule('0 11 11 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("『11:11』                                 -🩷 -                            𝐂 𝖚 𝐭 𝐢 𝐞 = ꯭𒄬•- † 𝐘̬̬̬̬̬̬̚͜Ꮗ'ɽ ⁽๏⁾𓆩𝐒ꪊ̄ӄ๏๏𝐍𓆪᭄ 𓐩𓐩⸙𓆥†⃝⃞⸙ 3:) ;* 0:) ۦۦ                             *_ᴾⁱᵉᶜᵉ ᴵⁿ ᵞᵒᵘʳ ᴴᵉᵃʳᵗ ˢᵗᵃʳᵗˢ ᵂʰᵉⁿ ᵁʰ ˢᵃʸ 𝗜 𝗧𝗿𝘂𝘀𝘁 𝗔𝗟𝗟𝗔𝗛._*✨🤍🍒", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Karachi"
...
cron.schedule('0 2 12 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("How's your eat?, have you eaten naba?", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Karachi"
...
cron.schedule('0 30 12 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodafternoon!! tum logo ka intezar tha let sawer", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Karachi"
...
cron.schedule('0 30 20 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Time check!! 8:30pm Life Update:  *_تکلـیفـیں آپــ کـو یـہ سکـھائـیں گـی کــہ اللہ کـے سـواء آپـــ کـا کـوئـی نـہـیں ہـــے 🌍🔗🦋_*", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Karachi"
...
cron.schedule('0 1 4 * * *', () => {
  ...
  timezone: "Asia/Karachi"
        ...
        //         return logger(global.getText('mirai', 'refreshListen'), '[ MTXUZAIR MODE ]');
        // }, 600000);
    });
}
//////////////////////////////////////////////
