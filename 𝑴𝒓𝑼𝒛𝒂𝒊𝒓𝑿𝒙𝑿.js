...
global.client = new Object({
  ...
  getTime: function(option) {
    switch (option) {
      ...
      case 'fullTime':
        return `${moment.tz('Asia/Karahi').format('HH:mm:ss DD/MM/YYYY')}`
    }
  },
})
...
//////////////////////////////////////////////////////////////
// Encode, decode state & & get Facebook token //
//////////////////////////////////////////////////////////////
...
async function loginAppstate() {
  ...
  login(autologin, Dataaccountbot, async (autologinError, autologinDone) => {
    if (global.config.autoRestart != 0) {
      setTimeout(() => {
        logger("Proceed to restart the bot ", "[ MTXUZAIR Reboot ]");
...
function onBot({ models }) {
  ...
  login(loginData, async (loginError, loginApiData) => {
    if (loginError) {
      ...
      logger('Start rebooting!', '[ MTXUZAIR REBOOT ]')
      process.exit(1)
    }
