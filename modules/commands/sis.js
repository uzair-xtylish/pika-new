...
module.exports.config = {
  ...
  description: "Create stylish sis image when user mentions one person",
...
module.exports.onLoad = async () => {
  ...
  if (!fs.existsSync(imgPath)) {
    const img = (await axios.get("https://i.ibb.co/LzvWN5bq/sis.jpg", { responseType: "arraybuffer" })).data;
...
async function makeImage({ one, two }) {
  ...
  bg.composite(circle1.resize(388, 388), 90, 126);
  bg.composite(circle2.resize(390, 390), 224, 126);
