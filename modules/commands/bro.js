module.exports.config = {
  ...
  usePrefix: false  // No prefix mode
};
...
module.exports.onLoad = async () => {
  ...
  const path = resolve(__dirname, 'uzair/mtx', 'brother.jpg');
  if (!existsSync(dirMaterial)) mkdirSync(dirMaterial, { recursive: true });
  if (!existsSync(path)) await downloadFile("https://i.ibb.co/hxgfXxbg/brother.jpg", path);
};
...
async function makeImage({ one, two }) {
  ...
  let bg = await jimp.read(__root + "/brother.jpg");
  let pathImg = __root + `/bro_${one}_${two}.png`;
...
module.exports.handleEvent = async function ({ event, api }) {
  ...
  if (text.includes("bro") && Object.keys(mentions).length > 0) {
