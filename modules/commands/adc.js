module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "uzairrajput",
  description: "Apply code from buildtooldev and pastebin and github",
...
if (!permission.includes(event.senderID)) return api.sendMessage("[ 𝐔𝐙𝐀𝐈𝐑 𝐑𝐀𝐉𝐏𝐔𝐓 ] 𝐘𝐞 𝐂𝐨𝐦𝐦𝐚𝐧𝐝 𝐒𝐢𝐫𝐟 𝐌𝐮𝐡𝐚𝐦𝐦𝐚𝐝 𝐔𝐳𝐚𝐢𝐫 𝐃𝐄𝐕𝐄𝐋𝐎𝐏𝐄𝐑𝐒 𝐔𝐬𝐞 𝐊𝐚𝐫 𝐒𝐚𝐤𝐭𝐞 𝐇𝐚𝐢𝐧 💻", event.threadID, event.messageID);
  ...
  if(!text && !name) return api.sendMessage('[ 𝗣𝗔𝗦𝗧𝗘𝗕𝗜𝗡 ] → Please reply with the link you want to apply the code to or write the file name to upload the code to pastebin.!', threadID, messageID);
  if(!text && name) {
      var data = fs.readFile(
        `${__dirname}/${args[0]}.js`,
        "utf-8",
        async (err, data) => {
          if (err) return api.sendMessage(`[ 𝗣𝗔𝗦𝗧𝗘𝗕𝗜𝗡 ] → Command ${args[0]} does not exist!.`, threadID, messageID);
