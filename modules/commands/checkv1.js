...
const tt = (yesterDay, toDay, time) => new Promise((resolve) => {
	if(yesterDay == 0) resolve("There are no statistics yet");
	if(toDay == 0) resolve("There are no statistics yet");
...
module.exports.run = async ({ api, event, args, Currencies, Users }) => {
	...
	if(args[0] == "all") {
		...
		msg += `\n» Total number of box messages in the past week: ${sum}`;
		...
		return api.sendMessage(`=== ${nameUser} ===\n: ${infoUser.exp}\n🐸 Level of interaction:${msg}\n⏱️ Total number of messages yesterday: ${yesterDay}\n⏰ Total number of messages per week: ${today}\n🖤 Total number of messages today: ${weekDay}`, threadID, messageID);
...
module.exports.config = {
	...
	credits: "uzairrajput", // Công nhận module sở hữu là ai
	description: "JUST CHECKTT", // Thông tin chi tiết về lệnh
	commandCategory: "For users", // Thuộc vào nhóm nào: system, other, game-sp, game-mp, random-img, edit-img, media, economy, ...
	usages: "[all]", // Cách sử dụng lệnh
	cooldowns: 5, // Thời gian một người có thể lặp lại lệnh
};
...
module.exports.run = async ({ api, event, args, Currencies, Users }) => {
	...
	if(args[0] == "all") {
		...
		return api.sendMessage(`=== ${nameUser} ===\n🙆‍♂️ total number of messages: ${infoUser.exp}\n🐸 Level of interaction: ${msg}\n⏱️ Total number of messages yesterday:${yesterDay}\n⏰ Total number of messages per week: ${today}\n🖤 Total number of messages today: ${weekDay}`, threadID, messageID);
	} else {
		...
		return api.sendMessage(`=== ${nameUser} ===\n🙆‍♂️ total number of messages: ${(typeof countMess.exp == "undefined") ? 0 : countMess.exp}\n🐸 Level of interaction: ${msg}\n⏱️ Total number of messages yesterday: ${yesterDay}\n⏰ Total number of messages per week: ${today}\n🖤 Total number of messages today: ${weekDay}`, threadID, messageID);
	}
}
		const countMess = await Currencies.getData(senderID);
		const yesterDay = data[threadID][time3].user[senderID].yesterDay;
